...
export function BlockItem({ block, isFocused, onUpdate, onAdd, onDelete, onFocus, onTypeChange, readOnly }: BlockItemProps) {
  ...
  useEffect(() => {
    if (isFocused && textareaRef.current && !readOnly) {
  ...
  const renderContent = () => {
    ...
    try {
      ...
      return <div className="p-2 text-xs text-destructive bg-destructive/10 rounded">Error rendering block</div>;
    }
  };
  return (
    <div
      ...
      style={style}
      className={cn(
        "group relative flex items-start gap-1 px-1 py-0.5 rounded-sm transition-colors",
        !readOnly && "hover:bg-zinc-50 dark:hover:bg-zinc-900/50",
